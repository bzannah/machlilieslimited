...
### Added in the content/SEO pass (`seo` branch) ✅
- **Services pillar hub** at `/services/` — a `CollectionPage` + `ItemList` that links
  every service and industry page, consolidating internal-link authority and giving
  head terms ("AI consulting", "agentic AI services", "AI experts") a real landing.
- **Guides & resources hub** at `/resources/` (`CollectionPage` + `ItemList`) plus three
  long-form articles, each with `Article`/`TechArticle` + `BreadcrumbList` + `FAQPage`
  schema (the pilots playbook also carries `HowTo`):
  - `/resources/what-is-agentic-ai-consulting/` → *agentic AI consulting, agentic consulting, AI experts*
  - `/resources/move-ai-pilots-to-production/` → *production-ready AI agents, AI pilot rescue*
  - `/resources/governed-ai-agents/` → *governed AI agents, AI governance consulting*
- **`Person` schema** for the three named principals on the homepage, with stable `@id`s
  reused as article author entities — a coherent cross-page entity graph (E-E-A-T).
- **Internal linking**: "All services" + "Guides & resources" added to every footer;
  articles cross-link to the relevant service pages with descriptive anchors.
- **Sitemap** updated with the five new URLs; **robots.txt** disallows the orphaned
  `archive_*.html` prototypes; **llms.txt** gained Services-hub + Guides sections (GEO).
- All additions are generated by `tools/generate_service_pages.py` (or are additive-only
  CSS / hand-edits to `index.html`), so the existing design and pages are untouched.

**Follow-up batches (also on `seo`, merged to `main`):**
- **Five more guides** in the hub (8 total): `how-to-choose-an-ai-consultancy`,
  `how-much-does-ai-consulting-cost`, `measuring-ai-agent-roi` (+`HowTo`),
  `ai-governance-checklist` (+`HowTo`), `agentops-vs-mlops` — covering buyer-intent,
  pricing, ROI, governance and comparison queries.
- **`/about/` page** (`AboutPage` + `BreadcrumbList` + `FAQPage` + three `Person` nodes
  sharing the homepage `@id`s) — E-E-A-T, targets "AI experts".
- **Bidirectional internal mesh**: every service & industry page now carries a
  "Further reading" block linking to its most relevant guides (`SERVICE_GUIDES` map),
  complementing the in-article links back to services. "About the team" added to footers.
- `<noscript>` fallback + `article:*` Open Graph on the generated hub/article pages.
...
- **Content cadence + off-page authority.** The multi-page architecture and the first
  content cluster now exist (services pillar + 3 guides). The remaining levers are the
  hardest and most decisive: a steady insights cadence (§7) and referring-domain growth
  (§8). Also pending and owner-dependent: real social `sameAs` URLs, GSC/Bing
  verification, and a Google Business Profile (§2) — these need real accounts, so they
  are flagged for the owner rather than committed as placeholders.
